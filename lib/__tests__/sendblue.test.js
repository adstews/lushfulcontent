...
import { sendMessage, getAccount, normalizePhone } from '../sendblue.js'
...
describe('getAccount', () => {
  it('GETs /accounts with auth headers', async () => {
    const fetchSpy = vi.spyOn(global, 'fetch').mockResolvedValue({
      ok: true,
      json: async () => ({ name: 'Lushful' })
    })
    const result = await getAccount()
    expect(fetchSpy).toHaveBeenCalledWith(
      'https://api.sendblue.co/api/accounts',
      expect.objectContaining({
        method: 'GET',
        headers: expect.objectContaining({
          'sb-api-key-id': 'key_test',
    ...
    expect(result).toEqual({ name: 'Lushful' })
  })
  ...
  it('throws on non-OK response', async () => {
    ...
    await expect(getAccount()).rejects.toThrow('SendBlue account check failed: 500')
  })
})
