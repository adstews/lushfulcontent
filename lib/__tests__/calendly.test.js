...
import { verifySecret, parseInviteeCreated } from '../calendly.js'

describe('verifySecret', () => {
  it('accepts a matching ?secret= query param', () => {
    expect(verifySecret({ url: '/api/calendly-webhook?secret=abc', headers: {} }, 'abc')).toBe(true)
  })
  it('accepts a matching X-Webhook-Secret header', () => {
    expect(verifySecret({ url: '/api/calendly-webhook', headers: { 'x-webhook-secret': 'abc' } }, 'abc')).toBe(true)
  })
  it('rejects a wrong secret', () => {
    expect(verifySecret({ url: '/api/calendly-webhook?secret=nope', headers: {} }, 'abc')).toBe(false)
  })
  it('rejects when no secret is present', () => {
    expect(verifySecret({ url: '/api/calendly-webhook', headers: {} }, 'abc')).toBe(false)
  })
  it('fails closed when the expected secret is unset', () => {
    expect(verifySecret({ url: '/api/calendly-webhook?secret=abc', headers: {} }, '')).toBe(false)
  })
})
