...
describe('POST /api/imessage/inbound', () => {
  ...
  it('STOP + known lead → suppress, unenroll, log; no pause', async () => {
    findLeadByPhone.mockResolvedValue({ closeLeadId: 'lead_x', contactId: null, displayName: 'X' })
    logImessageActivity.mockResolvedValue({ ok: true })
    suppressPhone.mockResolvedValue(undefined)
    unenrollAllForLead.mockResolvedValue(undefined)
    const { req, res } = makeReqRes({ event: 'message.received', sender: '+15550100123', text: 'STOP' })
    await handler(req, res)
    expect(res.statusCode).toBe(200)
    expect(suppressPhone).toHaveBeenCalledWith(expect.objectContaining({ phone: '+15550100123', leadId: 'lead_x' }))
    expect(unenrollAllForLead).toHaveBeenCalledWith('lead_x', expect.any(String))
